...
let ExtractFlowsFromSolution =
    let EmptyFolder =
        Directory.GetFiles(workflowPath, "*.*", SearchOption.AllDirectories)
    ...
    let ExtractAndUnzip =
        ...
        use archive =
            ZipFile.Open(workflowPath + flowSolutionName + ".zip", ZipArchiveMode.Read)
    ...
    let OrganizeFiles =
        ...
        let deleteNonFlows file =
            match file with
            | file when file.ToString().EndsWith(".json") ->
                  printf "Keeping: %s\n" file
